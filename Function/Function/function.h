...
public:                            // Секция доступных извне методов
    ...
    virtual double calculate(double x) const = 0; // Виртуальная функция для вычисления f(x)
    virtual string getName() const = 0;           // Виртуальная функция, возвращающая название функции
